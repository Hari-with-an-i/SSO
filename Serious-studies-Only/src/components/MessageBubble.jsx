...
const MessageBubble = ({ msg, isSender, onReply, onLongPress, isSelected, onMediaClick }) => {
            ...
            case 'video':
                ...
                return (
                    <div onClick={() => isThumbnailReady && onMediaClick(mediaProps)} className="relative max-w-xs cursor-pointer">
                        <img src={mediaProps.imageUrl} referrerPolicy="no-referrer" className={`rounded-lg w-full transition-opacity duration-300 ${isThumbnailReady ? 'opacity-100' : 'opacity-0'}`} onLoad={() => setIsThumbnailReady(true)} alt={msg.fileName} />
    ...
    return (
        <div className={`relative mb-4 flex items-center ${isSender ? 'justify-end' : 'justify-start'}`}>
            <div className="absolute top-1/2 -translate-y-1/2 text-3xl opacity-50" style={{ [isSender ? 'right' : 'left']: 'auto', right: isSender ? '-50px' : 'auto', left: isSender ? 'auto' : '-50px' }}>
                ↩️
            </div>
            <animated.div {...bindGestures()} style={{ x, touchAction: 'pan-y' }} className={`relative z-10 transition-colors duration-200 rounded-3xl ${isSelected ? (isSender ? 'bg-yellow-400/30' : 'bg-white/30') : ''}`}>
